...
//    QObject *itemSmile = 0;
//    QObject *itemTxt = 0;
    //QGraphicsItemGroup *nodeitem = 0;
    NodeItem *nodeitem = 0;
    int type = action->data().toInt();
    if (type != NodeItemType)
        return;
    ...
    itemSmile = new SmileyItem(QPoint((-SmileySize+xsize)/2,ysize/2), scene);
    itemBox = new BoxItem(QRect(QPoint(xsize/2,ysize/2), QSize(120, 60)), scene);
    //another smart way to handle this 3 types obj?
    nodeitem->addToGroup(dynamic_cast<QGraphicsItem*>(itemBox));
