...
bool SmileyItem::contains(const QPointF &point) const
{
    bool ret;
    //reimp, true if point in this item,
    //check if the current mouse point (in scene XY) is inside the EditTxt box
    //EditTxt Rect(x,y,w,h)
    QRectF rect(this->x()-SmileyHalfSize, this->y()-SmileyHalfSize, SmileySize,
                SmileySize);

    ret=rect.contains(point);
    return ret;
}
